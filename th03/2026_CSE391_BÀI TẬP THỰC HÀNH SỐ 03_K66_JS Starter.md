...
> - Hiện thị thông báo trong console:Hello from JavaScript!
> - 
---
...
> - JavaScript chạy ở Cả trình duyệt và server
> - HTML:Xây dựng cấu trúc và nội dung thô cho trang web
> - CSS: Định dạng kiểu dáng, màu sắc và cách trình bày giao diện.
> - JavaScript: Xử lý logic, tính toán và các tương tác động của người dùng.
...
>   - `string` là: Kiểu chuỗi ký tự (văn bản), luôn nằm trong dấu ngoặc  kép
>   - `boolean` là: Kiểu logic, chỉ có hai giá trị là true hoặc false
...
> - Dòng log hiển thị: "Xin chào, mình là Viet, năm nay mình 22 tuổi.".
> - Nếu quên dấu `;` hoặc`+`: Trình duyệt sẽ báo lỗi Uncaught SyntaxError (lỗi có màu đỏ) và đoạn mã sẽ ngừng chạy.
---
...
>   - Thân hàm (body): .........................................
>   - Giá trị trả về (return):kết quả được trả ra sau từ khóa return
> - Ưu điểm của việc dùng hàm thay vì lặp lại cùng một đoạn code nhiều lần là gì?
  - Ưu điểm:Giúp code gọn gàng, tránh lặp lại cùng một logic nhiều lần, dễ sửa lỗi.
...
> - Trong đoạn code trên, khi nhấn nút `btnHello`, điều gì thay đổi trên trang?
  - `document.getElementById("status")` Dùng để truy cập phần tử HTML có ID là "status".
  - Xảy ra khi người dùng nhấn chuột vào phần tử đó.
---
...
> - Khi em xoá hết nội dung ô input, dòng `greeting` hiển thị gì?
  - click" chỉ xảy ra khi bấm chuột, còn "input" xảy ra liên tục ngay khi người dùng gõ hoặc xóa bất kỳ ký tự nào trong ô nhập liệu.
  - Khi xóa hết nội dung: Dòng greeting sẽ chỉ hiển thị "Xin chào, !"
...
> - Điểm giống nhau về chức năng giữa 2 đoạn code trên là gì?
  - Giống nhau về chức năng:
     ...
     Cả hai đoạn code đều dùng để lắng nghe sự kiện click của người dùng trên một nút bấm có ID là btnHello và hiển thị một hộp thoại thông báo (alert).
> - Điểm khác nhau về cú pháp là gì (`document.getElementById` vs `$("#id")`, `addEventListener` vs `.on`)?
  - Khác nhau về cú pháp:

    JS thuần: Sử dụng phương thức dài và tường minh là document.getElementById để chọn phần tử và addEventListener để bắt sự kiện.

   jQuery: Sử dụng ký hiệu đặc trưng $ cùng với bộ chọn theo kiểu CSS ("#id") và phương thức .on rất ngắn gọn.
> - Em hãy tra cứu nhanh “What is jQuery used for?” và ghi 2 ý chính:
  1. Đơn giản hóa thao tác DOM: Giúp việc chọn các phần tử HTML, xử lý sự kiện và tạo hiệu ứng (ẩn/ hiện, trượt) trở nên dễ dàng với ít dòng code hơn.

  2. Tương thích đa trình duyệt: Đảm bảo mã JavaScript hoạt động ổn định trên nhiều trình duyệt  khác nhau mà không cần viết các đoạn mã kiểm tra điều kiện phức tạp.
...
> 1. Sau buổi lab, em tò mò nhất về phần nào của JavaScript/DOM?
  - Phần tò mò nhất:JavaScript có thể thay đổi giao diện web ngay lập tức mà không cần tải lại trang

> 2. Em muốn tự làm thêm tính năng gì trên trang web (vd: bộ đếm, đổi theme, pop-up, mini game, …)?
  - Tính năng muốn làm thêm: Em muốn tự làm một nút "Đổi giao diện Sáng/Tối" cho trang web của mình.
